...
func (a *App) GetUserOrder(username string) []user.IOrderUser {
	return a.userStore.GetOrderUser(username)
}
func (a *App) GetUser(userName string) ([]user.IUser, bool) { return a.userStore.GetUser(userName) }
func (a *App) GetUserCreateTime(userName string) *time.Time {
	return a.userStore.GetUserCreateTime(userName)
}
...
func (a *App) SendGroupTextMessage(from, groupId, msg string) int32 {
	...
	if groupId == "all" || groupId == "All" {
		a.userStore.RangeAllUser(func(username string) bool {
			if fa, _ := a.msgDock.SendMessage(message.NewTextMessage(from, username, msg)); fa {
				num++
			}
			return true
		})
...
func (a *App) GetHandle(name string) (RegisterHandle, bool) {
	h, ok := a.handleFuncMap[name]
	return h, ok
}
